...
public class Model {
    ...
    private void saveState(Tile[][] gameTiles) {
        ...
        previousMaxTile.push(maxTile);
        isSaveNeeded = false;
    }

    public void rollback() {
        if (!previousStates.isEmpty() && !previousScores.isEmpty() && !previousMaxTile.isEmpty()) {
            gameTiles = previousStates.pop();
            score = previousScores.pop();
            maxTile = previousMaxTile.pop();
        }
    }
    ...
    void intelligentAutoMove() {
        PriorityQueue<MoveEfficiency> priorityQueue = new PriorityQueue<>(Collections.reverseOrder());
        int count = 5;
        priorityQueue.add(intelligentGetMoveEfficiency(new PriorityQueue<>(Collections.reverseOrder()), this::left, count, this::left));
        priorityQueue.add(intelligentGetMoveEfficiency(new PriorityQueue<>(Collections.reverseOrder()), this::right, count, this::right));
        priorityQueue.add(intelligentGetMoveEfficiency(new PriorityQueue<>(Collections.reverseOrder()), this::up, count, this::up));
        priorityQueue.add(intelligentGetMoveEfficiency(new PriorityQueue<>(Collections.reverseOrder()), this::down, count, this::down));
        priorityQueue.peek().getMove().move();
    }

    MoveEfficiency intelligentGetMoveEfficiency(PriorityQueue<MoveEfficiency> priorityQueue, Move move, int count, Move nextMove) {
        priorityQueue.add(new MoveEfficiency(-1, 0, move));
        nextMove.move();
        if (hasBoardChanged()) {
            if (count > 0) {
                count--;
                intelligentGetMoveEfficiency(priorityQueue, move, count, this::left);
                intelligentGetMoveEfficiency(priorityQueue, move, count, this::right);
                intelligentGetMoveEfficiency(priorityQueue, move, count, this::up);
                intelligentGetMoveEfficiency(priorityQueue, move, count, this::down);

            } else {
                priorityQueue.add(new MoveEfficiency(getEmptyTilesCount(), score, move));
            }
        }
        rollback();
